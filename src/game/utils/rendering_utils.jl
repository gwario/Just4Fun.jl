...
YELLOW_DOMINATED(text) = string(crayon"fg:(0,0,0) bg:default !bold", text, crayon"reset")
...
RED_DOMINATED(text) = string(crayon"fg:(0,0,0) bg:default !bold", text, crayon"reset")
...
GREEN_DOMINATED(text) = string(crayon"fg:(0,0,0) bg:default !bold", text, crayon"reset")
...
MAGENTA_DOMINATED(text) = string(crayon"fg:(0,0,0) bg:default !bold", text, crayon"reset")
...
default_style(player::Player) =
    ...
    player == MAGENTA ? MAGENTA_DEFAULT : player == GREEN ? GREEN_DEFAULT : throw(AssertionError("Unhandled player"))

dominated_style(player::Player) =
    player == YELLOW ? YELLOW_DOMINATED :
    player == RED ? RED_DOMINATED :
    player == MAGENTA ? MAGENTA_DOMINATED :
    player == GREEN ? GREEN_DOMINATED : throw(AssertionError("Unhandled player"))

dominating_style(player::Player) =
    player == YELLOW ? YELLOW_DOMINATING :
    player == RED ? RED_DOMINATING :
    player == MAGENTA ? MAGENTA_DOMINATING :
    player == GREEN ? GREEN_DOMINATING : throw(AssertionError("Unhandled player"))

majority_style(player::Player) =
    player == YELLOW ? YELLOW_MAJORITY :
    player == RED ? RED_MAJORITY :
    player == MAGENTA ? MAGENTA_MAJORITY : player == GREEN ? GREEN_MAJORITY : throw(AssertionError("Unhandled player"))

minority_style(player::Player) =
    player == YELLOW ? YELLOW_MINORITY :
    player == RED ? RED_MINORITY :
    player == MAGENTA ? MAGENTA_MINORITY : player == GREEN ? GREEN_MINORITY : throw(AssertionError("Unhandled player"))

player_name(p::Player) = p == YELLOW ? "Yellow" : p == RED ? "Red" : p == MAGENTA ? "Magenta" : "Green"

padded(s; front = true, length = 2, padding_char = ' ') =
    front ? lpad(s, length, padding_char) : rpad(s, length, padding_char)
...
function print_player_stones(g::Just4FunEnv, pos::Tuple{Int64,Int64}, p::Player)
    ...
    if iszero(p_stones)
        ...
        if has_majority(all_player_stones, p) && !dominating(all_player_stones, p)
            style = majority_style(p)
        elseif has_minority(all_player_stones, p) && !dominated(all_player_stones, p)
...
function print_playercards(spec::Just4FunSpec, g::Just4FunEnv)
    name_max_len = max([length(player_name(Player(i))) for i = 1:spec.settings.players]...)
...
function print_curplayer_actions(spec::Just4FunSpec, g::Just4FunEnv)
    ...
    card_actions = Just4Fun.cards_actions(spec, g, g.curplayer)
...
function print_gamecards(g::Just4FunEnv)
    println(
        "Unused cards: (top most card) ",
        join(convert(Vector{Int64}, reverse(collect(Iterators.reverse(g.stack)))), " "),
    )
end
...
function Base.println(io::IO, s::Just4FunSpec, offset = 0)
    ...
    println(
        io,
        spaces,
        "Starting player: $(isnothing(s.starting_player) ? "random" : convert(Int64, s.starting_player))",
    )
